...
{
    internal class StreamDeskDBControl
    {
        ...
        public static void Initialize()
        {
            ...
            StreamDeskDB = new SQLiteConnection(String.Format("Data Source={0};Compress=True;Synchronous=Off", path2));
            StreamDeskDB.Open();
        ...
        public static string Search(string searchParms)
        {
            List<string> woot = new List<string>();
            var xml = "<searchresults>";

            var cmd = new SQLiteCommand(StreamDeskDB);
            cmd.CommandText = String.Format("SELECT * FROM Streams WHERE (Name Like '%{0}%')", searchParms);
            SQLiteDataReader reader = cmd.ExecuteReader();
            while (reader.Read())
            {
                var stripme = String.Format(@"<stream Name=""{0}"" Description=""{1}"" Provider=""{2}"" />", reader["Name"], reader["Description"], reader["Provider"]);
                if (!woot.Contains(stripme))
                {
                    woot.Add(stripme);
                    xml += stripme;
                }
            ...
            cmd.CommandText = String.Format("SELECT * FROM Streams WHERE (Provider Like '%{0}%')", searchParms);
            ...
            cmd.CommandText = String.Format("SELECT * FROM Streams WHERE (Description Like '%{0}%')", searchParms);
            reader = cmd.ExecuteReader();
            while (reader.Read())
            {
                var stripme = String.Format(@"<stream Name=""{0}"" Description=""{1}"" Provider=""{2}"" />", reader["Name"], reader["Description"], reader["Provider"]);
        ...
        public static bool Update()
        {
            try
            {
                ...
                m.DownloadFile(downloadpath, path2);
